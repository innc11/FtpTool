...
class File:
    ...
    def copyTo(self, dist):
        ...
        assert self.exists, f"{self.absPath} is not found"
    ...
    def relativePath(self, baseDir=None):
        if baseDir is None:
        ...
        raise NotADirectoryError(f"'The base dir '{bd.path}' is not a Directory")

    def append(self, relPath):

        assert isinstance(relPath, str), "relPath must be a str object"
